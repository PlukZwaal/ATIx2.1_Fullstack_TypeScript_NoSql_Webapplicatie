  ...
  <nav class="bg-white/80 backdrop-blur-md border-b border-red-100/60 sticky top-0 z-50 shadow-sm">
    <div class="container mx-auto px-6 lg:px-8">
      <div class="flex justify-between items-center h-16">
        ...
        <div v-if="authStore.isAuthenticated" class="hidden md:flex items-center gap-8 absolute left-1/2 transform -translate-x-1/2">
        ...
        <button v-if="authStore.isAuthenticated" @click="menuOpen = !menuOpen" class="md:hidden text-slate-600 hover:text-slate-900 transition-colors p-2" aria-label="Menu">
          <svg :class="{ 'hidden': menuOpen }" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div v-if="authStore.isAuthenticated && menuOpen" class="md:hidden border-t border-slate-200 py-4 space-y-2 bg-white/95 backdrop-blur-sm">
        <router-link @click="closeMenu" to="/modules" class="block px-4 py-3 text-slate-700 hover:text-blue-600 hover:bg-blue-50 transition-colors rounded-lg mx-2">
          Modules
        </router-link>
        <router-link @click="closeMenu" to="/modules/create" class="block px-4 py-3 text-slate-700 hover:text-blue-600 hover:bg-blue-50 transition-colors rounded-lg mx-2">
          Nieuwe Module
        </router-link>
        ...
        <div class="border-t border-slate-200 pt-4 mt-4 mx-2">
          <div class="flex items-center justify-between px-4 py-3 bg-slate-50 rounded-lg">
            ...
            <button @click="handleLogoutAndClose" class="text-red-600 hover:text-red-700 transition-colors">
              Uitloggen
            </button>
          </div>
        </div>
